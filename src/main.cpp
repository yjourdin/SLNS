...
int main(int argc, char* argv[]) {
  ...
  std::vector<DestroyOperator> small_destroy_operators = {RandomRemoval()};
  std::vector<DestroyOperator> large_destroy_operators = {RandomRemoval()};
  std::vector<RepairOperator> small_repair_operators = {Random()};
  std::vector<RepairOperator> large_repair_operators = {Random()};
  iteration_count LNS_freq = 10 * std::pow(instance.get_nb_clients(), 1.5);
